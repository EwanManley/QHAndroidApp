...
public class UserAccount {
    ...
    private static final String ANON =
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1wdnR0ampwd2doeXlkZnVtcXhpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTcwNTAzODcsImV4cCI6MjA3MjYyNjM4N30.IUkEutAeR0fDZswjXXduZu2CyZJ5eNt9KvCaF0ax9DE";
        ...
        try {
            ...
            http.newCall(req).enqueue(new Callback() {
                @Override public void onFailure(Call call, java.io.IOException e) {
                    cb.onResult(false, "Network error: " + safe(e), null);
                }
                @Override public void onResponse(Call call, Response response) {
                    try {
                        String raw = response.body() == null ? "" : response.body().string();
                        Log.d(TAG, "signup raw: " + raw);
                        if (!response.isSuccessful()) {
                            cb.onResult(false, "Sign-up HTTP " + response.code() + " " + trim(raw, 160), null);
                            return;
                        }
                        ...
                        if (TextUtils.isEmpty(token)) {
                            cb.onResult(false, "Please verify your email to activate the account.", null);
                            return;
                        }
                        ...
                        cb.onResult(false, "Sign-up parse error", null);
                    }
                }
            });
        } catch (Exception e) {
            cb.onResult(false, "Sign-up build error", null);
        }
    }
    ...
    public static void login(Context ctx, String email, String password, AuthCallback cb) {
        try {
            ...
            http.newCall(req).enqueue(new Callback() {
                @Override public void onFailure(Call call, java.io.IOException e) {
                    cb.onResult(false, "Auth network error: " + safe(e), null);
                }
                @Override public void onResponse(Call call, Response response) {
                    try {
                        String raw = response.body() == null ? "" : response.body().string();
                        Log.d(TAG, "login raw: " + raw);
                        if (!response.isSuccessful()) {
                            cb.onResult(false, "Auth HTTP " + response.code() + " " + trim(raw, 160), null);
                            return;
                        }
                        JSONObject obj = new JSONObject(raw);
                        String token = obj.optString("access_token", null);
                        if (TextUtils.isEmpty(token)) {
                            cb.onResult(false, "No token in response", null);
                            return;
                        }
                        ...
                        cb.onResult(false, "Auth parse error", null);
                    }
                }
            });
        } catch (Exception e) {
            cb.onResult(false, "Auth build error", null);
        }
    }

    private static void fetchProfile(Context ctx, String email, String token, AuthCallback cb) {
        try {
            String url = BASE + "/rest/v1/profile?select=role,council&email=eq." +
                    URLEncoder.encode(email, StandardCharsets.UTF_8.name()) + "&limit=1";
            ...
            http.newCall(req).enqueue(new Callback() {
                @Override public void onFailure(Call call, java.io.IOException e) {
                    cb.onResult(false, "Profile network error: " + safe(e), null);
                }
                @Override public void onResponse(Call call, Response response) {
                    try {
                        String raw = response.body() == null ? "" : response.body().string();
                        Log.d(TAG, "profile raw: " + raw);
                        if (!response.isSuccessful()) {
                            cb.onResult(false, "Profile HTTP " + response.code() + " " + trim(raw, 160), null);
                            return;
                        }
                        ...
                        cb.onResult(false, "Profile parse error", null);
                    }
                }
            });
        } catch (Exception e) {
            cb.onResult(false, "Profile build error", null);
        }
    }
    ...
    private static void upsertProfile(Context ctx, String email, String role, String council, String token, AuthCallback cb) {
        try {
            ...
            http.newCall(req).enqueue(new Callback() {
                @Override public void onFailure(Call call, java.io.IOException e) {
                    cb.onResult(false, "Profile upsert network error: " + safe(e), null);
                }
                @Override public void onResponse(Call call, Response response) {
                    try {
                        String raw = response.body() == null ? "" : response.body().string();
                        Log.d(TAG, "profile upsert raw: " + raw);
                        if (!response.isSuccessful()) {
                            cb.onResult(false, "Profile upsert HTTP " + response.code() + " " + trim(raw, 160), null);
                            return;
                        }
                        ...
                        cb.onResult(false, "Profile upsert parse error", null);
                    }
                }
            });
        } catch (Exception e) {
            cb.onResult(false, "Profile upsert build error", null);
        }
    }
